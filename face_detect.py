...
def main():
    ...
    # emotionetfolder = '/mnt/disk50/datasets/emotionet_95w/'
    # outputfolder = '/mnt/disk50/datasets/emotionet_face/'
    inputfolder = '/home/xintian/MORPH/Images_ori/'
    outputfolder = '/home/xintian/MORPH/Output_head/'
    minsize = 20
    ...
    # imglistfolders = os.listdir(inputfolder)
    # for imglistfolder in imglistfolders[11:]:
    #     if imglistfolder[:8] == "dataFile" and imglistfolder[-3:] != 'AUs' and imglistfolder[-1] != ')':
    #         imgpath = inputfolder + imglistfolder
    ...
    for i in range(total):
        ...
        img = cv2.imread(inputfolder + imgName)
        if img is not None:
            img_matlab = img.copy()
            tmp = img_matlab[:,:,2].copy()
            img_matlab[:,:,2] = img_matlab[:,:,0]
    ...
    #            cv2.rectangle(img, (int(boundingboxes[i][0]), int(boundingboxes[i][1])), (int(boundingboxes[i][2]), int(boundingboxes[i][3])), (0,255,0), 1)
            ...
            if len(boundingboxes) > 0 and len(points) > 0:
                face, bboxes = crop_face(img, boundingboxes, points)
                # print('bboxes: {}' .format(bboxes))
                # cv2.imshow('img', img)
                # cv2.imwrite('test/demo{}.png'.format(cnt), img)
                face = np.array(face)
                # print(face)
                # if not os.path.exists(outputfolder + imglistfolder):
                #     os.makedirs(outputfolder + imglistfolder)
                # cv2.imwrite(outputfolder + imglistfolder + '/' + imgname[:-4] + '.png', face)
                if not os.path.exists(outputfolder):
                    os.makedirs(outputfolder)
                cv2.imwrite(outputfolder + imgName[:-4] + '.png', face)
                # ch = cv2.waitKey(0) & 0xFF
                # if ch == 27:
                #     break
        sys.stdout.write('{0}/{1}\r'.format(i+1, total))
        sys.stdout.flush()
        time.sleep(0.01)
...
def run_emotionet():
    filetxt = './emotionet_new.txt'
    datasetPathOut = os.path.join('/', 'mnt', 'disk50', 'datasets', 'emotionet_face_new')
    ...
    with open(filetxt, 'r') as fp:
        imgList = fp.read().splitlines()
        total = len(imgList)
        cnt = 0
        for i in range(total):
            imgPath = imgList[i]
            ...
            if img is not None and img.shape[0] > 256 and img.shape[1] > 256:
                ...
                if len(boundingboxes) > 0 and len(points) > 0:
                    face, bboxes = crop_face(img, boundingboxes, points)
                    face = np.array(face)
                    cv2.imwrite(os.path.join(datasetPathOut, imgName[:-4] + '.png'), face)
            progress_bar(i, total)
        print('{} images have been saved!'.format(cnt))
